...
class Geometry(object):
    """
    Class to generate nodes and members for Semi-Parallel Chord Roof Truss geometry
    """
    def __init__(self, span, height, nVertWebsPerSide=1, trussDepthMiddle=None, trussDepthEnd=None):
        self.span = span
        self.height = height # height of the bottom chord, total truss height will be height + trussDepthMiddle
        self.trussDepthMiddle = span / 10 if trussDepthMiddle is None else trussDepthMiddle
        self.trussDepthEnd = span / 12 if trussDepthEnd is None else trussDepthEnd
    ...
    def getNodes(self):
        ...
        for i in range(0, nSpansTop + 1):
            xit = horSpacingTop * i
            top_chord_rise = self.height + (self.trussDepthMiddle - self.trussDepthEnd)
            hit = min(xit * 2 * top_chord_rise / self.span, 2 * top_chord_rise - xit * 2 * top_chord_rise / self.span) + self.trussDepthEnd
